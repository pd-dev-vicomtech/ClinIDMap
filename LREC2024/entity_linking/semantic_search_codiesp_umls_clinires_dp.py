...
df_train = pd.read_csv(os.path.join(train_dir, train_file), sep='\t', dtype=str).fillna('')
...
df_train = df_train.loc[df_train['label'] == 'DIAGNOSTICO' ]
...
# sab_values = ['ICD10_WIKI', 'ICD10CM_WIKI', 'ICD10', 'ICD10PCS_WIKI', 'ICD10PCS', 'ICD10CM', 'ICD10CM_SPA', 'ICD10PCS_SPA']
sab_values = ['ICD10_WIKI', 'ICD10CM_WIKI', 'ICD10', 'ICD10PCS_WIKI', 'ICD10CM', 'ICD10CM_SPA', 'SNOMEDCT2ICD10_SPA']
...
## VECTORS
output_folder = 'SapBERT_codiesp_clinires_diag'
...
for n in ns: 
    ...
    for item in tqdm.tqdm(result):
        ...
        # print(item['true'])
        # if item['code_true'] in top_n_icd[:n]:
        
        if any(i in top[:n] for i in item['true']):
            top_n.append(1)
        else:
            top_n.append(0)
        # print(str(item['predicted_labels'][0]))
        df_cui_to_icd10 = df_database[(df_database['CUI'] == str(item['predicted_labels'][0])) & (df_database['SAB'].isin(sab_values))]
        # print(item['code_true'])
        # print(df_cui_to_icd10.head())
        # df_cui_to_icd10_ = df_database[df_database['CUI'] == str(item['predicted_labels'][0])]
        # print(df_cui_to_icd10_.head())
        # print('++++++++++++++++++++++')
